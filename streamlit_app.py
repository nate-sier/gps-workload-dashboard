...
def classify_status_from_snapshot(row, criteria=None):
    ...
    # PP_ACWR gates the combined-load escalation. High practice + High game
    # stays as context unless the sheet ACWR is elevated (>= monitor threshold).
    if combined == "Major Load Concern" and (pd.isna(acwr) or acwr < criteria["monitor_acwr"]):
        combined = "High Practice + Game Load"
