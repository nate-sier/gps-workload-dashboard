...
# PP_ACWR_COMBINED_LOAD_FIX = "2026-08-13-v2"
...
# Combined High-practice + High-game load only escalates to Review when
# that PP_ACWR value is elevated (>= 1.30); combined load alone cannot Review.
...
def classify_combined_load(practice_level, game_load_class, acwr_val=np.nan, elevated_threshold=1.30):
    """Combined-load matrix gated by PP_ACWR (the authoritative ACWR source).

    High practice + High game is only a Major Load Concern when the PP_ACWR
    value is elevated. A low/missing PP_ACWR keeps the label Practice-Driven
    Spike, while the underlying GPS z-score can still independently drive Review.
    """
    gl = game_load_class if game_load_class in ("High", "Moderate", "Low") else "Low"
    acwr = pd.to_numeric(acwr_val, errors="coerce")
    if practice_level == "High" and gl == "High":
        if pd.notna(acwr) and acwr >= float(elevated_threshold):
            return "Major Load Concern"
        return "Practice-Driven Spike"
    if practice_level == "High":
        return "Practice-Driven Spike"
...
def classify_status_from_snapshot(row, criteria=None):
    ...
    combined = classify_combined_load(
        practice_level, game_class, acwr_val=acwr,
        elevated_threshold=criteria["monitor_acwr"],
    )
    ...
    if (criteria["use_combined_load"] and combined == "Major Load Concern"
            and criteria["use_acwr"] and pd.notna(acwr) and acwr >= criteria["monitor_acwr"]):
        return "Review", f"High combined practice + game load + elevated ACWR ({acwr:.2f})", "Check soreness/readiness; avoid additional sprint volume tomorrow", combined, practice_level
